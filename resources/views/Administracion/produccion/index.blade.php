...
<body class="sb-nav-fixed">
    ...
    <div id="layoutSidenav">
        ...
        <div id="layoutSidenav_content">
                            ...
                            <table class="table" id="proccTable">
                                <thead>
                                    <tr>
                                        <th>Id</th>
                                        <th>Fecha</th>
                                        <th>Cantidad de Fundas</th>
                                        <th>Producto</th>
                                        <th>Acciones</th>
                                    </tr>
                                </thead>
                                <tbody id="proccTableBody">
                                    @forelse ($produccions as $produccion)
                                        <tr>
                                            <td>{{ $produccion->id }}</td>
                                            <td>{{ $produccion->fecha ?? '' }}</td>
                                            <td>{{ $produccion->cantidad_fundas ?? '' }}</td>
                                            <td>{{ $produccion->producto ? $produccion->producto->nombre_producto : 'N/A' }}</td>
                                            <td>
                                                <div class="btn-group" role="group">
                                                    <button class="btn btn-success btn-editProcc"
                                                        data-id="{{ $produccion->id }}"
                                                        data-fecha="{{ $produccion->fecha ?? '' }}"
                                                        data-cantidad-fundas="{{ $produccion->cantidad_fundas ?? '' }}"
                                                        data-producto="{{ $produccion->producto_id ?? '' }}">
                ...
                <div class="modal-dialog">
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <form method="POST" action="{{ route('produccion.store') }}" style="padding: 15px;">
                                @csrf
                                <div class="row">
                                    <div class="col-md-6 mb-3">
                                        <label for="fechaProcc" class="form-label">Fecha:</label>
                                        <input type="date" class="form-control" name="Fecha" required>
                                    </div>
                                    <div class="col-md-6 mb-3">
                                        <label for="cantidadFundasProcc" class="form-label">Cantidad de Fundas:</label>
                                        <input type="number" class="form-control" name="CantidadFundas" min="0" required>
                                    </div>
                                </div>
                                <div class="row">
                                    <div class="col-md-12 mb-3">
                                        <label for="productoProcc" class="form-label">Producto:</label>
            ...
            <!-- Modal de Produccion Editar -->
            <div class="modal fade" id="editModalProcc" tabindex="-1" aria-labelledby="editModalProccLabel"
                aria-hidden="true">
                <div class="modal-dialog">
                    <div class="modal-content">
                        <form method="POST" id="editProccForm" action="">
                            @csrf
                            @method('PUT')
                            <div class="modal-header">
                                <h5 class="modal-title" id="editModalProccLabel">Editar Producción</h5>
                                <button type="button" class="btn-close" data-bs-dismiss="modal"
                            ...
                            <div class="modal-body">
                                <div class="row">
                                    ...
                                    <div class="col-md-6 mb-3">
                                        <label for="editCantidadFundasProcc" class="form-label">Cantidad de Fundas:</label>
                                        <input type="number" class="form-control" id="editCantidadFundasProcc"
                                            name="CantidadFundas" min="0" required>
                                    </div>
                                </div>
                                <div class="row">
                                    <div class="col-md-12 mb-3">
                                        <label for="editProductoProcc" class="form-label">Producto:</label>
                                        <select class="form-select" id="editProductoProcc" name="Producto" required>
            ...
            <div class="modal fade" id="deleteModalProcc" tabindex="-1" aria-labelledby="deleteModalProccLabel"
                ...
                <div class="modal-dialog">
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <form method="POST" id="delete-form-procc" action="">
                                @csrf
                                @method('DELETE')
    ...
    <!-- Script modal editar -->
    <script>
        $(document).on('click', '.btn-editProcc', function() {
            const editForm = document.getElementById('editProccForm');
            const modal = new bootstrap.Modal(document.getElementById('editModalProcc'));

            const button = this;
            const id = button.getAttribute('data-id');
            const fecha = button.getAttribute('data-fecha') || '';
            const cantidadFundas = button.getAttribute('data-cantidad-fundas') || '';
            ...
            document.getElementById('editCantidadFundasProcc').value = cantidadFundas;
    ...
    <script>
        $(document).on('click', '.btn-deleteProcc', function() {
            ...
            const button = this;
            const id = button.getAttribute('data-id');
            deleteForm.setAttribute('action', `/produccion/${id}`);
            deleteModal.show();
        });
    </script>
</body>
